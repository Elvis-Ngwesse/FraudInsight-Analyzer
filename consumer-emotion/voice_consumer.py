...
INFLUXDB_BUCKET_VOICE_EMOTION = get_env_var("INFLUXDB_BUCKET")
...
def on_message(channel, method, properties, body):
    ...
    try:
        ...
        emotions = emotion_classifier(transcript)[0]  # list of dicts
        ...
        point = (
            ...
            .field("top_emotion", top_emotion["label"])
            .field("top_emotion_score", round(top_emotion["score"], 4))
            .time(datetime.utcnow(), WritePrecision.NS)
        )

        for emo in emotions:
            point = point.field(f"emotion_{emo['label']}", round(emo["score"], 4))

        write_api.write(bucket=INFLUXDB_BUCKET_VOICE_EMOTION, record=point)
        logging.info(f"Wrote emotion point for message {message_id}")
...
def main():
    ensure_bucket_exists(influx_client, INFLUXDB_BUCKET_VOICE_EMOTION, INFLUXDB_ORG)
